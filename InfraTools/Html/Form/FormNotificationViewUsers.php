...
<table class='TableGeneric'>
	<tr>
		<form  name="{$FM_NOTIFICATION_VIEW_LST_USERS_FORM}" method="{$FORM_METHOD}">
			<th class="TableGenericThArrow">
				<div class="TableGenericInputLeft">
					<input  type="image" class="TableGenericThArrowImage"
							name="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_BACK}" id="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_BACK}" 
							value="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_BACK}" title="{$SUBMIT_BACK}" alt="{$SUBMIT_BACK}"
			...
			<th  class="TableGenericThArrow">
				...
				<div class="TableGenericInputRight">
					<input  type="image" class="TableGenericThArrowImage"
							name="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_FORWARD}" id="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_FORWARD}" 
							value="{$FM_NOTIFICATION_VIEW_LST_USERS_SB_FORWARD}" title="{$SUBMIT_FORWARD}" alt="{$SUBMIT_FORWARD}"
		...
		{foreach key=key item=item from=$INSTANCE_USER}
			<tr>
				<td class='TableGenericTdLink'>
					<form  name="{$FM_NOTIFICATION_SEL}" method="{$FORM_METHOD}">
						<input type='hidden' name="{$FM_NOTIFICATION_SEL_SB}" id="{$FM_NOTIFICATION_SEL_SB}" 
						       value="{$FM_NOTIFICATION_SEL_SB}" />
						<input type='submit' name="FIELD_NOTIFICATION_ID" id="FIELD_NOTIFICATION_ID" 
						       value="{$FIELD_NOTIFICATION_ID_VALUE}" title="{$FIELD_NOTIFICATION_ID_VALUE}" maxlength="5" />
					</form>
				</td>
				<td class="TableGenericTdLink">
					<form name="{$FM_USER_SEL_SB}" method="{$FORM_METHOD}">
						<input type="hidden" name="{$FM_USER_SEL_SB}" id="{$FM_USER_SEL_SB}"
							   value="{$FM_USER_SEL_SB}">
						<input type="submit" name="{$FIELD_USER_EMAIL}" id="{$FIELD_USER_EMAIL}"
		                       value="{$item->GetEmail()}" title="{$item->GetEmail()}" maxlength="80" />	
					</form>
				</td>
				<td>
					{$item->GetName()}
				...
				<td class="TableGenericTdLink">
					<form name="{$FM_TYPE_USER_SEL_SB}" method="{$FORM_METHOD}">
						<input type="hidden" name="{$FM_TYPE_USER_SEL_SB}" id="{$FM_TYPE_USER_SEL_SB}"
							   value="{$FM_TYPE_USER_SEL_SB}">
						<input type="submit" name="{$FIELD_TYPE_USER_DESCRIPTION}" id="{$FIELD_TYPE_USER_DESCRIPTION}"
							   value="{$item->GetUserTypeDescription()}" title="{$item->GetUserTypeDescription()}" maxlength="80" />	
					</form>
				</td>
				{if isset($item->GetCorporationName())}
					<td class="TableGenericTdLink">
						<form name="{$FM_CORPORATION_SEL_SB}" method="{$FORM_METHOD}">
							<input type="hidden" name="{$FM_CORPORATION_SEL_SB}" id="{$FM_CORPORATION_SEL_SB}"
								value="{$FM_CORPORATION_SEL_SB}">
							<input type="submit" name="{$FIELD_CORPORATION_NAME}" id="{$FIELD_CORPORATION_NAME}" 
								value="{$item->GetCorporationName()}" title="{$item->GetCorporationName()}" maxlength="80" />	
						</form>
					</td>
				{else} 
					<td>
				...
				{if isset($item->GetDepartmentName())}
					<td class="TableGenericTdLink">
						<form name="{$FM_DEPARTMENT_SEL_SB}" method="{$FORM_METHOD}">
							<input type="hidden" name="{$FM_DEPARTMENT_SEL_SB}" id="{$FM_DEPARTMENT_SEL_SB}"
								   value="{$FM_DEPARTMENT_SEL_SB}">
							<input type="hidden" name="{$FIELD_CORPORATION_NAME}" id="{$FIELD_CORPORATION_NAME}"
								   value="{$item->GetCorporationName()}">
							<input type="submit" name="{$FIELD_DEPARTMENT_NAME}" id="{$FIELD_DEPARTMENT_NAME}"
								   value="{$item->GetDepartmentName()}" title="{$item->GetDepartmentName()}" maxlength="80" />	
						</form>
					</td>
				{else} 
					<td>
