...
<table class='TableGeneric'>
	<tr>
		<form  name="{$FM_TYPE_SERVICE_LST_FORM}" method='post'>
			<th class="TableGenericThArrow">
				<div class="TableGenericInputLeft">
					<input  type="image" class="TableGenericThArrowImage"
							name="{$FM_TYPE_SERVICE_LST_BACK}" 
							id="{$FM_TYPE_SERVICE_LST_BACK}" 
							value="{$FM_TYPE_SERVICE_LST_BACK}"
				...
				<div class="TableGenericThRight">
					{$FIELD_TYPE_SERVICE_NAME_TEXT}
				</div>
			</th>
			<th  class="TableGenericThDiv">
				{$FIELD_TYPE_SERVICE_SLA_TEXT}
			</th>
			<th  class="TableGenericThArrow">
				...
				<div class="TableGenericInputRight">
					<input  type="image" class="TableGenericThArrowImage"
							name="{$FM_TYPE_SERVICE_LST_FORWARD}" 
							id="{$FM_TYPE_SERVICE_LST_FORWARD}" 
							value="{$FM_TYPE_SERVICE_LST_FORWARD}"
	...
	{foreach name=outer from=$ARRAY_INSTANCE_INFRATOOLS_TYPE_SERVICE item=INSTANCE_TYPE_SERVICE}
		{foreach key=key item=item from=$INSTANCE_TYPE_SERVICE}
			<tr>
				<td class="TableGenericTdLink">
					<form name="{$FM_TYPE_SERVICE_SEL_SB}" method="post">
						<input type="hidden" 
						       name="{$FM_TYPE_SERVICE_SEL_SB}" 
							   id="{$FM_TYPE_SERVICE_SEL_SB}"
							   value="{$FM_TYPE_SERVICE_SEL_SB}"/>
						<input type="submit" 
						       name="{$FIELD_TYPE_SERVICE_NAME}" 
							   id="{$FIELD_TYPE_SERVICE_NAME}"
							   value="{$item->GetTypeServiceName()}"
							   title="{$item->GetTypeServiceName()}"
							   maxlength="80" />	
					</form>
				</td>
				<td class="TableGenericTdLink">
					{$item->GetTypeServiceSLA()}
				</td>
				<td class="TableGenericTdLink">
